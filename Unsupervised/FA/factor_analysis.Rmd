...
# Note: minor differences from the 2019 Gibson et al paper are due to the updated psych package
```
...
```{r}
loadings <- as_tibble(cbind(rownames(fa_4_p$loadings[]), fa_4_p$loadings[])) %>% 
  rename(Variable = V1) %>% 
...
plot_loadings %>% 
  ...
  coord_flip() + geom_hline(yintercept = 0, linewidth = 0.2)
